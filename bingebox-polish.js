(() => {
  ...
  function clean(){
    qsa('body > div:not(#__next):not(.bb-search-overlay):not(.bb-loading-note),next-route-announcer').forEach(e=>e.remove());
    ...
    const more=qsa('button').find(e=>e.textContent.trim()==='More Movies');if(more){more.textContent='Browse all titles';more.classList.add('bb-browse-all');more.onclick=()=>location.href='/lite';}
