(() => {
  ...
  function buildHover(card){
    ...
    try{const fav=JSON.parse(localStorage.getItem('bb-exact-favorites')||'[]');if(data.slug&&Array.isArray(fav)&&fav.includes(data.slug)){save.classList.add('rs-collected','HoverCard_collected__83dCr')}}catch{}
    ...
    save.addEventListener('click',e=>{
      ...
      let fav=[];try{fav=JSON.parse(localStorage.getItem('bb-exact-favorites')||'[]')}catch{}
      const set=new Set(Array.isArray(fav)?fav:[]);
      if(slug){if(set.has(slug))set.delete(slug);else set.add(slug);localStorage.setItem('bb-exact-favorites',JSON.stringify([...set]));}
