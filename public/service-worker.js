...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname==='/watch.js'){event.respondWith(fetch(req,{cache:'no-store'}).then(res=>{if(res.ok){const clone=res.clone();caches.open(CACHE).then(c=>c.put(req,clone))}return res}).catch(()=>caches.match(req)));return;}\n  if(req.mode==='navigate'){
